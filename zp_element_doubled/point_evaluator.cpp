...
static const int factor[7] = {1,1,2,6,24,120,720};
using namespace std;
			...
			{	
				...
				for(int col_num=0;col_num<4;col_num++)
				{
					if(NULL != cell)
					{
						...
						p[row_num][col_num][i][j]=cp[0][row_num][col_num][i][j]/cp[1][row_num][col_num][i][j];
						//on<<p[row_num][row_num][i][j]<<" ";
					}
					else
					{
...
//	{
//		for(int yn=0; yn+xn<=6; yn++) 
//		{
//			int wn = 6-xn-yn;
//			int f = factor[6]/(factor[xn]*factor[yn]*factor[wn]);
//			for(int i=0;i<6;i++)
//			{
//					for(int j=0;j<6;j++)
//				{
...
double point_evaluator::evaluate(double *y)
{
  ...
  //bary为三维矩阵inv_V中i行的值之和，bary 为重心坐标
  for(int i=0;i<3;i++)
  {
  ...
  for(int i=0;i<=6;i++)
  {
    for(int j=0;j<=(6-i);j++)
    {
      ...
      temp2 = p[n][triangle_index][i_square[0]+3][i_square[1]+3];
	  temp3 = my_pow(bary[1],j);
	  temp4 = my_pow(bary[0],i);
	  temp5 = my_pow(bary[2],k);
	  temp = temp2*720.0/(factorial(i)*factorial(j)*factorial(k))*temp3*temp4*temp5;
